...
const ExtrasPage = () => {
  ...
  const handleImageClick = (id: string) => {
    setSelected(id);
    router.push(`/productos/extras?item=${id}`);
  };

  const closeOverlay = () => {
    setSelected(null);
    router.push("/productos/extras");
  };

  const shareUrl =
    typeof window !== "undefined"
      ? `${window.location.origin}/productos/extras?item=${selected}`
      : "";
      ...
      <div className="w-full p-2 pb-4 sm:px-5 lg:px-7 flex justify-start">
        ...
        <div className="">
          <Button variant="secondary">
            <a href="https://photos.app.goo.gl/1N4gbQAGh7RgYTJR7" target="_blank" rel="noopener noreferrer" className="flex gap-2 cursor-pointer text-sm">
