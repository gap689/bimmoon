...
const portaplacasImages = Array.from({ length: 14 }, (_, i) => ({
  id: `portaplaca-${i + 1}`,
  src: `${i + 1}.jpg`,
  alt: `Portaplaca ${i + 1}`
}));

const PortaplacasPage = () => {
  ...
  const handleImageClick = (id: string) => {
    setSelected(id);
    router.push(`/productos/portaplacas?item=${id}`);
  };

  const closeOverlay = () => {
    setSelected(null);
    router.push("/productos/portaplacas");
  };

  const shareUrl =
    typeof window !== "undefined"
      ? `${window.location.origin}/productos/portaplacas?item=${selected}`
      : "";
    ...
    <div className="flex flex-col items-center justify-center pt-16">
      <div className="w-full p-2 pb-4 sm:px-5 lg:px-7 flex justify-start">
        ...
        <div className="">
          <Button variant="secondary">
            <a href="https://photos.app.goo.gl/ymMsTppnzXVLXU9RA" target="_blank" rel="noopener noreferrer" className="flex gap-2 cursor-pointer text-sm">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {portaplacasImages.map((img) => (
          ...
          >
            <Image
              src={`/images/portaplacas/${img.src}`}
      ...
      <AnimatePresence>
        {selected && (
          ...
          >
            <motion.img
              src={`/images/portaplacas/${selected}`}
              alt={selected}
...
export default PortaplacasPage;