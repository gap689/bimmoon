...
const marimbasImages = Array.from({ length: 27 }, (_, i) => ({
  id: `portaplaca-${i + 1}`,
  src: `${i + 1}.jpg`,
  alt: `Portaplaca ${i + 1}`
}));
...
const MarimbasPage = () => {
  ...
  const handleImageClick = (id: string) => {
    setSelected(id);
    router.push(`/productos/marimbas?item=${id}`);
  };

  const closeOverlay = () => {
    setSelected(null);
    router.push("/productos/marimbas");
  };

  const shareUrl =
    typeof window !== "undefined"
      ? `${window.location.origin}/productos/marimbas?item=${selected}`
      : "";
      ...
      <div className="w-full p-2 pb-4 sm:px-5 lg:px-7 flex justify-start">
        ...
        <div className="">
          <Button variant="secondary">
            <a href="https://photos.app.goo.gl/rY7cWV4E3UE7RGzx6" target="_blank" rel="noopener noreferrer" className="flex gap-2 cursor-pointer text-sm">
